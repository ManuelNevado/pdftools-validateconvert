...
/**
 * @brief Prepares the invoice XML file (ZUGFeRD or Factur-X) for embedding.
...
* @param[in] iInvoiceType The type of invoice.

* @param[in] pInvoice The XML invoice stream.

* @param[in] pAfRelationship If no value is provided, a sensible default value is chosen based on the invoice type and
version.
 ...
 * - \ref ePdfTools_Error_IllegalArgument The invoice stream could not be opened for reading.


 */
PDFTOOLS_EXPORT BOOL PDFTOOLS_CALL PdfToolsPdfAConversion_Converter_AddInvoiceXml(
    TPdfToolsPdfAConversion_Converter* pConverter, TPdfToolsPdfAConversion_InvoiceType iInvoiceType,
    const TPdfToolsSys_StreamDescriptor* pInvoice, const TPdfToolsPdfAConversion_AFRelationship* pAfRelationship);
/**
 ...
 * - \ref ePdfTools_Error_Processing Failed to add the invoice file.
...
/**
 * @brief Image quality of recompressed images
...
Example:
JPX (JPEG2000) is not allowed in PDF/A-1. If a PDF contains a JPX compressed image, its compression type must be
altered. Thus the image is converted to an image with JPEG compression using the image quality defined by this property.
Default value: `0.8`
 ...
 * Get the error message with \ref PdfTools_GetLastErrorMessage.
 * Possible error codes:
 * - \ref ePdfTools_Error_IllegalArgument The given value is smaller than `0.1` or greater than `1`.


 */
PDFTOOLS_EXPORT double PDFTOOLS_CALL
PdfToolsPdfAConversion_ConversionOptions_GetImageQuality(TPdfToolsPdfAConversion_ConversionOptions* pConversionOptions);
/**
 * @brief Image quality of recompressed images
...
* @param[in] dImageQuality Set value.
 ...
 */
PDFTOOLS_EXPORT BOOL PDFTOOLS_CALL PdfToolsPdfAConversion_ConversionOptions_SetImageQuality(
    TPdfToolsPdfAConversion_ConversionOptions* pConversionOptions, double dImageQuality);
